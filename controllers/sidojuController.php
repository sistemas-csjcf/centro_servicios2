...
/*---------- Mensajes -------------*/
	...
	// Función de filtrar documentos según el juzgados
		...
		if($_SESSION['id']!=""){
				...
				//if($nivel == 1){
					...
					if($tienepermiso == 1){
						...
						print'<script languaje="Javascript">alert("�Acceso denegado para este m�dulo, verifique la configuraci�n de su perfil con el administrador del sistema...!"); location.href="index.php?controller=index&action=ruta_base"</script>';
	...
	public function Editar_documento_Entrante_Juzgado_Salud(){
		...
		public function Documento_Entrante_Juzgado_Salud_Ver(){
		if($_SESSION['id']!=""){
			require 'models/sidojuModel.php';
			$modelo  = new sidojuModel();
			if($_GET){
			...
			$this->view->show("sidoju_documentos_entrantes_juzgados_salud_ver.php", $data);
	  	} else{
			header("refresh: 0; URL=/centro_servicios2/");
		}
	}
