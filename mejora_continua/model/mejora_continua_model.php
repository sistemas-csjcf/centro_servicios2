    ...
    public function get_Responsables(){
        ...
        public function Registrar_Tarea(Mejora_C $data){
            ...
            try {
                ...
                if($data->tipo_us ==1){
                    $sql = "INSERT INTO `mc_tareas`(`tar_fecha`, `tar_id_user`, `tar_id_clase`, 
                                `tar_id_numeral`, `tar_descripcion`,`tar_id_proceso_responsable`, `tar_id_proceso_afectado`, `tar_causa`, 
                                `tar_id_metodologia`, `tar_id_generada`, `tar_estado`) 
                        VALUES (?,?,?,?,?,?,?,?,?,?,?)";
                    ...
                    $sql = "INSERT INTO `mc_tareas`(`tar_fecha`, `tar_id_user`, `tar_descripcion`,
                        `tar_id_proceso_responsable`, `tar_estado`) 
                        VALUES (?,?,?,?,?)";
                    $this->pdo->prepare($sql)
                    ->execute(
                        array(
        ...
        public function Completar_AG($data){
            try{
                //$proc_impactados = implode(",", $data->id_procesoImp);
                $sql = "INSERT INTO `mc_tareas_detalles`(`det_tar_id`,`det_fecha_inicial`, `det_fecha_final`, `det_descripcion`, `det_responsable`, `tar_ruta_doc`) VALUES (?,?,?,?,?,?)";
