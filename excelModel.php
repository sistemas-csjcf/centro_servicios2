   ...
   public function Datos_Permisos_AprobadosNoAprobadosEnProceso($usuariop,$fdp,$fhp,$estadop){
			...
			$listar    = $this->db->prepare("SELECT ep.id,pu.empleado,ep.fecha_solicitud,ep.fecha_permiso,ep.hora_inicio,ep.hora_final,ep.detalle,ep.estado,
											  ...
											  CASE
												...
												AS duracion, TIMESTAMPDIFF(MINUTE , concat(ep.fecha_permiso,' ',ep.hora_inicio), concat(ep.fecha_permiso,' ',ep.hora_final) )/60 AS duracion_m
...
************************************ en una fecha ************************************/
	...
	if($juzgado=='Todos'){	
		$oficios = $this->db->prepare("select concat(correspondencia_otros.destinatario,' ',radicado,'/',correspondencia_otros.oficio_telegrama) as nomdestino
,concat(pa_municipio.nombre,' - ',pa_departamento.nombre) as ciudad,correspondencia_otros.direccion as direccion,
CONCAT( pa_juzgado.nombre,  '/', pa_juzgado.cod_usuario_juzgado ) AS observaciones
...
where correspondencia_otros.esOficio_Telegrama='Oficio' and (correspondencia_otros.fecha   BETWEEN '$fecha' and '$fechaf' )
and correspondencia_otros.idmedionotificacion in (2,4,5) ORDER BY correspondencia_otros.oficio_telegrama");
       }
	   else
	    {
		 $oficios = $this->db->prepare("select concat(correspondencia_otros.destinatario,' ',radicado,'/',correspondencia_otros.oficio_telegrama) as nomdestino
...
inner join pa_juzgado on (pa_juzgado.id=correspondencia_otros.idjuzgado)
where correspondencia_otros.esOficio_Telegrama='Oficio' and (correspondencia_otros.fecha   BETWEEN '$fecha' and '$fechaf' )
and correspondencia_otros.idmedionotificacion in (2,4,5) and correspondencia_otros.idjuzgado in $lista ORDER BY correspondencia_otros.oficio_telegrama");		
		}
	...
	if($juzgado=='Todos'){			
	$oficios1 = $this->db->prepare("select concat(acc.accionante_accionado_vinculado,' ',ct.radicado,'/',actuacion.oficio_telegrama) as nomdestino,
...
where actuacion.esOficio_Telegrama='Oficio' and (actuacion.fecha_envio BETWEEN '$fecha' and '$fechaf')
and actuacion.idmedionotificacion in (2,4,5) ORDER BY actuacion.oficio_telegrama");
    } 
	else
	{
	$oficios1 = $this->db->prepare("select concat(acc.accionante_accionado_vinculado,' ',ct.radicado,'/',actuacion.oficio_telegrama) as nomdestino,
...
inner join pa_juzgado on (pa_juzgado.id=ct.idjuzgado)
where actuacion.esOficio_Telegrama='Oficio' and (actuacion.fecha_envio BETWEEN '$fecha' and '$fechaf')
and actuacion.idmedionotificacion in (2,4,5) and ct.idjuzgado in  $lista  ORDER BY actuacion.oficio_telegrama");
	}
		$oficios1->execute();
...
************************************ en una fecha ************************************/
	...
	{	
		...
		if($juzgado=='Todos'){			
		$telegramas = $this->db->prepare("select concat(correspondencia_otros.destinatario,' ',radicado,'/',correspondencia_otros.oficio_telegrama) as nomdestino
,concat(pa_municipio.nombre,' - ',pa_departamento.nombre) as ciudad,correspondencia_otros.direccion as direccion,
...
inner join pa_departamento on (pa_departamento.id=pa_municipio.iddepartamento)
inner join pa_juzgado on (pa_juzgado.id=correspondencia_otros.idjuzgado)
where correspondencia_otros.esOficio_Telegrama='Telegrama' and (correspondencia_otros.fecha  BETWEEN '$fecha' and '$fechaf')
and correspondencia_otros.idmedionotificacion in (2,4,5) ORDER BY correspondencia_otros.oficio_telegrama");
 }
  else
  {
...
and correspondencia_otros.idmedionotificacion in (2,4,5) and correspondencia_otros.idjuzgado in $lista  ORDER BY correspondencia_otros.oficio_telegrama");
	...
	if($juzgado=='Todos'){			
		$telegramas1 = $this->db->prepare("select concat(acc.accionante_accionado_vinculado,' ',ct.radicado,'/',actuacion.oficio_telegrama) as nomdestino,
concat(pa_municipio.nombre,' - ',pa_departamento.nombre) as ciudad,actuacion.direccion as direccion,
pa_juzgado.nombre as observaciones
from actuacion_tutela as actuacion
inner join accionante_accionado_vinculado as acc ON (actuacion.idaccionado_vinculado_accionante_tut=acc.id)
...
inner join pa_departamento on (pa_departamento.id=pa_municipio.iddepartamento)
inner join pa_juzgado on (pa_juzgado.id=ct.idjuzgado)
where actuacion.esOficio_Telegrama='Telegrama' and (actuacion.fecha_envio  BETWEEN '$fecha' and '$fechaf')
and actuacion.idmedionotificacion in (2,4,5) ORDER BY correspondencia_otros.oficio_telegrama");
}
else
{
$telegramas1 = $this->db->prepare("select concat(acc.accionante_accionado_vinculado,' ',ct.radicado,'/',actuacion.oficio_telegrama) as nomdestino,
...
inner join pa_juzgado on (pa_juzgado.id=ct.idjuzgado)
where actuacion.esOficio_Telegrama='Telegrama' and (actuacion.fecha_envio  BETWEEN '$fecha' and '$fechaf')
and actuacion.idmedionotificacion in (2,4,5) and ct.idjuzgado in $lista ORDER BY correspondencia_otros.oficio_telegrama");
}
