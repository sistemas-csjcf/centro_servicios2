                ...
                <tr>
                    ...
                    <?php if( $r->vis_pro_estado =='Pendiente'){ ?>
                        <td><?php echo $r->vis_pro_estado ?>
                            <button class="btn btn-warning btn-xs" data-toggle="modal" data-target="#myModalNorm" data-id="<?php echo $r->vis_pro_id  ?>"data-idtsocial="<?php echo $r->vis_pro_id_TSocial ?>" ><span class="icon-cancel-circle"> Cancelar</span></button></td>
                    <?php }else if( $r->vis_pro_estado =='Aprobada' && $r->vis_pro_finalizada == 0){ ?>
                        <td>
                            ...
                            <?php } ?> 
                            <a href="?c=Visitas&a=Generar_Formato&dx=<?php echo $r->vis_pro_id; ?>" title="Generar Formato"><span class="glyphicon glyphicon-save"></span></a>
                        </td>
                    ...
                    <?php }else{ ?>
                        <td><span class="glyphicon glyphicon-remove-sign" style="color: #FF0000;"><?php echo $r->vis_pro_estado; ?></span>
                            <br><br><strong>Motivo Cancelación: </strong><?php echo $r->vis_pro_motivo; ?>
                        </td>
                    <?php } ?> 
                    <?php if($r->vis_pro_estado =='Pendiente'){ ?>
                        <td><p style="color: white; font-size: 1px">Pendiente</p></td>
                    <?php } else if($r->vis_pro_estado =='Aprobada'){ ?>
                        <td><p style="color: white; font-size: 1px">Aprobada</p></td>
                    <?php }else{ ?>
                        <td><p style="color: white; font-size: 1px">Cancelada</p></td>
                    <?php } ?>
                </tr>
            <?php endforeach; ?>
        </tbody>
