<?php
	...
	private $pdo;
        ...
        public function Listar(){
            try{
                $stm = $this->pdo->prepare("SELECT * FROM visitas_sin_aprobar");
                $stm->execute();
        ...
        public function Registrar(Visita $data){
            ...
            try {
                ...
                $sql = "INSERT INTO visitas_programacion 
                    (vis_pro_radicado,vis_pro_solicitante,vis_pro_fecha_visita, vis_pro_fecha_audiencia,vis_pro_comentarios,vis_pro_clase_proceso,vis_pro_subclase_proceso,vis_pro_datos_partes,vis_pro_id_TSocial,vis_pro_id_usuario,vis_pro_estado,vis_pro_finalizada) 
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
                $this->pdo->prepare($sql)
                ->execute(
                    array(
        ...
        public function Listar_Historial(){
            try{
                ...
                if ( in_array($_SESSION['idUsuario'],$usuariosa) ) {
                    $stm = $this->pdo->prepare("select prog_vis.vis_pro_id AS vis_pro_id,prog_vis.vis_pro_radicado AS vis_pro_radicado,prog_vis.vis_pro_solicitante AS vis_pro_solicitante,prog_vis.vis_pro_fecha_visita AS vis_pro_fecha_visita,prog_vis.vis_pro_fecha_audiencia AS vis_pro_fecha_audiencia,prog_vis.vis_pro_comentarios AS vis_pro_comentarios,prog_vis.vis_pro_clase_proceso AS vis_pro_clase_proceso,prog_vis.vis_pro_subclase_proceso AS vis_pro_subclase_proceso,prog_vis.vis_pro_datos_partes AS vis_pro_datos_partes,prog_vis.vis_pro_id_TSocial AS vis_pro_id_TSocial,prog_vis.vis_pro_id_usuario AS vis_pro_id_usuario,prog_vis.vis_pro_id_usuarioE AS vis_pro_id_usuarioE,prog_vis.vis_pro_estado AS vis_pro_estado,prog_vis.vis_pro_motivo AS vis_pro_motivo,prog_vis.vis_pro_finalizada AS vis_pro_finalizada,prog_vis.vis_pro_fecha_registro AS vis_pro_fecha_registro,tsoc.vis_TSoci_id_usuario AS vis_TSoci_id_usuario,tsoc.vis_TSoci_nombre AS vis_TSoci_nombre,tsoc.vis_TSoci_contador AS vis_TSoci_contador from (visitas_programacion prog_vis join visitas_trabajador_social tsoc on((prog_vis.vis_pro_id_TSocial = tsoc.vis_TSoci_id)))");
                }else{
        ...
        public function visitas_pendientes_allTS(){ 
            try{
                $stm = $this->pdo->prepare("SELECT * FROM `visitas_pendientes_allts` ");

               // $stm = $this->pdo->prepare("SELECT * FROM `visitas_pendientes_allts` WHERE vis_pro_id_TSocial =3 ");
                $stm->execute();
                return $stm->fetchAll(PDO::FETCH_OBJ);
            } catch (Exception $e) {
                die($e->getMessage());
            ...
            try {
                ...
                if($data->bool_log =="0"){
                    $sql = "UPDATE visitas_informe_seguimiento 
                        ...
                        vis_inf_num_visitas_realizadas = ?,
                        vis_inf_duracion = ?,
...
//            $fecha1 = "2017-06-01"; //            $fecha2 = "2017-06-06";
        ...
        public function Informe_Impreso($id){
            try {
                $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);        
                //EMPIEZA LA TRANSACCION
                $this->pdo->beginTransaction();
                $stm = $this->pdo
                ->prepare("UPDATE visitas_informe_remision 
                            SET inf_rem_impreso = 1
                            WHERE inf_rem_id = ?");
                $stm->execute(array($id));
        ...
        public function Listar_Historial_Valoracion_Visitas(){
            try{
                ...
                $id_usuario = $_SESSION['idUsuario'];
                $stm = $this->pdo->prepare("SELECT * FROM `Listar_Historial_Valoracion_Visitas` WHERE vis_pro_fecha_visita BETWEEN '2017-09-06' AND '$fechaActual'");
                $stm->execute();
                return $stm->fetchAll(PDO::FETCH_OBJ);
            } catch (Exception $e) {
                die($e->getMessage());
        ...
        public function Listar_All_Solicitudes_Visitas(){
            try{
                $id_usuario = $_SESSION['idUsuario'];
                $stm = $this->pdo->prepare("SELECT * 
                    FROM visitas_programacion AS pro
                    INNER JOIN visitas_trabajador_social AS ts ON pro.vis_pro_id_TSocial = ts.vis_TSoci_id ");

                /* $stm = $this->pdo->prepare("SELECT * 
                    FROM visitas_programacion AS pro
                    INNER JOIN visitas_trabajador_social AS ts ON pro.vis_pro_id_TSocial = ts.vis_TSoci_id
                    WHERE vis_pro_fecha_visita BETWEEN '2018-04-01' AND '2018-06-30' ");
                */
                $stm->execute();
                return $stm->fetchAll(PDO::FETCH_OBJ);
            } catch (Exception $e) {
