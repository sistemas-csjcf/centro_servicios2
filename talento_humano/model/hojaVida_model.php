<?php
	...
	private $pdo;
        ...
        public function RegistrarDatosPersonalesForaneo(HojaVida $data){
            ...
            try {
                ...
                $this->pdo->exec("INSERT INTO pa_usuario (`nombre_usuario`, `idperfil`, `tipo_perfil`, `empleado`, `contrasena`, `id_area_cs`,
                         `id_proceso_cs`, `bandera_hv`, `idareaempleado`, `idestadoempleado`, `ingreso`) 
                         VALUES ('$data->per_cedula', '23','empleado','$nombre_completo','xxxxx','15','12','1','7','0','0')");
        ...
        public function Listar_FormacionProfesional($id_hv){
            try{
                $listar = $this->pdo->prepare("SELECT `for_pro_id`,`for_pro_id_HV`, `for_pro_id_user`, `for_pro_id_nivel_educacion`, 
				                              `for_pro_titulo`,`for_pro_institucion`, `for_pro_fecha_inicio`, `for_pro_fecha_fin`, 
											  `for_pro_ruta_certificado`, niv_titulo FROM th_formacion_profesional AS est 
        ...
        public function Listar_Exp_Laboral($id_hv){
            $listar = $this->pdo->prepare("SELECT `exp_id`, `exp_id_hv`, `exp_id_usuario`, `exp_id_usuarioR`, `exp_id_departamento`, `exp_id_municipio`, `exp_rama_flag`, `exp_empresa`, `exp_cargo`, `exp_CS_flag`, `exp_id_area_CS`, `exp_fecha_inicio`, `exp_fecha_actualmente`, `exp_fecha_fin`, `exp_funciones`, `exp_ruta_certificado`, `exp_id_usuarioE`, dep.nombre AS departamento, mun.nombre AS municipio,
IF(  `exp_fecha_actualmente` =0,
TIMESTAMPDIFF( YEAR,  `exp_fecha_inicio` ,  `exp_fecha_fin` ) , TIMESTAMPDIFF( YEAR,  `exp_fecha_inicio` , NOW( ) ) ) AS anios, IF(  `exp_fecha_actualmente` =0, DATEDIFF(  `exp_fecha_fin` ,  `exp_fecha_inicio` ) , DATEDIFF( NOW( ) ,  `exp_fecha_inicio` ) ) AS dias
FROM `th_experiencia_laboral` AS exp
INNER JOIN `pa_municipio` AS mun ON mun.id = exp.exp_id_municipio
INNER JOIN `pa_departamento`  AS dep ON  dep.id= mun.iddepartamento
WHERE exp.`exp_id_hv`=$id_hv");
            $listar->execute();
            return $listar->fetchAll(PDO::FETCH_OBJ);
        }
