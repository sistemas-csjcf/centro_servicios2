<?php
	class wordModel extends modelBase{
		...
		public function Obtener_Datos_Documento($iddoc){
			...
			$listar     = $this->db->prepare("SELECT rds.id,rds.identrada,td.nombre_tipo_documento,td.id AS iddocumento,rds.numero,d.nombre_dirigido,rds.nombre,rds.cargo,rds.dependencia,rds.fechageneracion,rds.asunto,rds.contenido, rds.con_copia FROM ((sigdoc_documentos_internos rds LEFT JOIN sigdoc_pa_tipodocumento td ON rds.idtipodocumento = td	.id) LEFT JOIN sigdoc_pa_dirigido d ON rds.dirigidoa = d.id)
				WHERE rds.id = '$iddoc'");
		$listar->execute();		  
		return $listar; 
   	}
	...
	if($opcion == 1){
		...
		if($datos0b == 2){
			...
			if($datos8<='2019-12-31') {
				$section->addText('NATALIA SABOGAL ORTIZ', $fontStyle45, $paraStyle45);
			} else {
