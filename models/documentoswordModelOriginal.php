<?php
	class documentoswordModel extends modelBase{
		...
		$listar     = $this->db->prepare("SELECT di.id,pro.radicado,jo.id AS idjuzgado,jo.nombre,di.idtipodocumento,cpro.nombre_proceso,
										  pr.nombre AS demandado,pr.cedula AS cedulademandado,pr2.nombre AS demandante,pr2.cedula AS cedulademandante,pro.claseproceso2,pro.entidadcomisiona,pro.asunto,pro.despacholibra,di.direccion,di.ciudad
...
if($opcion == 1){
	...
	if ( in_array($idtd,$modulost2) ) {
		...
		while( $filar = $datosradicado->fetch() ){
				...
				$dator11  = $filar[claseproceso2];
				$dator12  = $filar[entidadcomisiona];
				$dator13  = $filar[asunto];
				$dator14  = $filar[despacholibra];
				$dator15  = $filar[direccion];
				$dator16  = $filar[ciudad];
		}
	}
	else{
...
//****************************** JEST***********
	...
	//NOTIFICACION PERSONAL DESPACHO COMISORIO(Radicado Manual)
	if($datos0c == 79){
		$header = $section->createHeader();
        $table  = $header->addTable();
        $table->addRow();
        $table->addCell(2000)->addImage('views/images/header_docs/enca32.png', array('width'=>629, 'height'=>100, 'align'=>'center'));
		$datospartesB = explode("//////",$datospartes);
		$parte1 = $datospartesB[1];//A SOLICITUD DE
		$parte2 = $datospartesB[2];//QUIEN NOTIFICA
		$parte3 = $datospartesB[3];//ANEXOS
		$parte4 = $datospartesB[4];//PROMOVIDA
		$section->addText(utf8_decode("NOTIFICACIÓN PERSONAL")."            "."RADICADO: ".$dator2, $fontStyleA, $paraStyleA2);
		$section->addTextBreak(1);
		...
		$section->addText("Manizales, ___________________; en la fecha el ".utf8_decode("señor(a)")." ".$parte2." con C.C. No. ______________, adscrito a la planta de personal del CENTRO DE SERVICIOS JUDICIALES PARA LOS JUZGADOS CIVILES Y DE FAMILIA, debidamente autorizado por la Coordinadora NATALIA QUINTERO HOYOS, y a solicitud del ".utf8_decode("señor(a)")." ".$parte1." de fecha ".$fa.utf8_decode(", se trasladó ")."a ".$dator15." de la ciudad ".$dator16." para notificar personalmente a el/la ".($datos2).". ".$dator5.".".$datos10,$fontStyleB, $paraStyleB2);
		$section->addTextBreak(1);
		$conttabla = 0;
		while($conttabla < 7){
			if($conttabla == 0){$campofila = "ASUNTO:              ";         $campofila2 = strtoupper($dator13);}
			if($conttabla == 1){$campofila = "DESPACHO QUE LIBRA:              ";   $campofila2 = strtoupper($dator14);}
			if($conttabla == 2){$campofila = "ENTIDAD QUE COMISIONA:"; $campofila2 = strtoupper($dator12);}
			if($conttabla == 3){$campofila = "CLASE:              ";                $campofila2 = strtoupper($dator11);}
			if($conttabla == 4){$campofila = "PROMOVIDA:     ";        $campofila2 = strtoupper($parte4);}
			if($conttabla == 5){$campofila = "NOTIFICADA:              ";           $campofila2 = strtoupper($dator5);}
			if($conttabla == 6){$campofila = "ANEXOS:              ";               $campofila2 = strtoupper($parte3);}
			
			$styleTable    = array('borderSize'=>5, 'borderBottomColor'=>'FFFFFF', 'borderInsideHColor'=>'FFFFFF', 'borderInsideVColor'=>'FFFFFF', 'borderLeftColor'=>'FFFFFF', 'borderRightColor'=>'FFFFFF', 'bgColor'=>'FFFFFF');
			$styleFirstRow = array('borderSize'=>5, 'borderBottomColor'=>'FFFFFF', 'borderTopColor'=>'FFFFFF', 'borderLeftColor'=>'FFFFFF', 'borderRightColor'=>'FFFFFF', 'bgColor'=>'FFFFFF');
			$PHPWord->addTableStyle('myOwnTableStyle', $styleTable, $styleFirstRow);$table1 = $section->addTable('myOwnTableStyle');
			$table1->addRow(500);
			$table1->addCell(2000, $styleCell)->addText($campofila,$fontStyleC, $paraStyleC2);
			$table1->addCell(8000, $styleCell)->addText($campofila2,$fontStyleC, $paraStyleC2);
			$conttabla = $conttabla + 1;
		...
		while($conttabla < 7){
			...
			if($conttabla == 1){$campofila = ""; $campofila2 = "c.c:                 _______________";}
			if($conttabla == 2){$campofila = ""; $campofila2 = "# de celular:  _______________";}
			if($conttabla == 3){$campofila = ""; $campofila2 = "Fecha:           _______________";}
			if($conttabla == 4){$campofila = ""; $campofila2 = utf8_decode("Dirección:    _____________________________________");}
			if($conttabla == 5){$campofila = "QUIEN NOTIFICA,"; $campofila2 = strtoupper($parte2);}
			if($conttabla == 6){$campofila = "COORDINADORA,";   $campofila2 = strtoupper($datoofi6);}
			$styleTable    = array('borderSize'=>5, 'borderBottomColor'=>'FFFFFF', 'borderInsideHColor'=>'FFFFFF', 'borderInsideVColor'=>'FFFFFF', 'borderLeftColor'=>'FFFFFF', 'borderRightColor'=>'FFFFFF', 'bgColor'=>'FFFFFF');
			$styleFirstRow = array('borderSize'=>5, 'borderBottomColor'=>'FFFFFF', 'borderTopColor'=>'FFFFFF', 'borderLeftColor'=>'FFFFFF', 'borderRightColor'=>'FFFFFF', 'bgColor'=>'FFFFFF');
			$PHPWord->addTableStyle('myOwnTableStyle', $styleTable, $styleFirstRow);
	...
	if($datos0c == 71){
		...
		while($conttabla < 3){
			...
			if($conttabla == 0){$campofila = "NOTIFICADO(A): ";   $campofila2 = strtoupper($dator5);}
			if($conttabla == 1){$campofila = "QUIEN NOTIFICA: ";  $campofila2 = strtoupper($_SESSION['nombre']);}
			if($conttabla == 2){$campofila = "COORDINADOR(A): "; $campofila2 = strtoupper($datoofi6);}
