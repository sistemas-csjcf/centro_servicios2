			...
			try {
				...
				$this->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					...
					/*$listar = $this->db->prepare("SELECT MAX(di.numero) AS idmaximo,d.sigla AS siglas
												  ...
												  //SE AGREGA LO DEL A�O YA QUE CADA A�O LOS CONSECUTIVOS DE LOS DOCUMENTOS SE DEBEN INICIALIZAR A CERO (0)
					...
					if( empty($iddocumento) ){
						...
						$lastId = "";
						$lstd = $this->db->prepare("
						SELECT id
						FROM documentos_internos
						WHERE idparte = '$idparte'
						AND idradicado = '$idr'
						AND idusuario = '$idusuario'
						AND numero = '$ndocumento'
						AND fechageneracion LIKE '$fechag'
						");
						$lstd->execute();
						while($field = $lstd->fetch()){
							$lastId = $field['id'];
					  }
						$anotacion = $anotacion.", ID DOCUMENTO: ".$lastId;

						//*******************************************************************************************************************************
						//CAPTURO LOS DATOS DE LA SIGLA, EL A�O ACTUAL Y ULTIMO ID REGISTRADO DEL ULTIMO INSERT EN LA TABLA documentos_internos
