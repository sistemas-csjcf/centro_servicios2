...
/** Se obtiene un vector de juzgados **/
	public function obtenerJuzgados() {
		$fechai   = $_POST['fechai'];
		$fechaf   = $_POST['fechaf'];
		$juzgados = $this->db->prepare("select DISTINCT co.idjuzgado, pa_juzgado.nombre from correspondencia_otros co  inner join pa_juzgado on (pa_juzgado.id=co.idjuzgado) where co.fecha  BETWEEN '$fechai' and '$fechaf' union select DISTINCT ct.idjuzgado,pa_juzgado.nombre from correspondencia_tutelas ct inner join accionante_accionado_vinculado av on (ct.id=av.idcorrespondencia_tutelas) inner join actuacion_tutela act on (act.idaccionado_vinculado_accionante_tut=av.id) inner join pa_juzgado on (pa_juzgado.id=ct.idjuzgado) where act.fecha_envio BETWEEN '$fechai' and '$fechaf' order by idjuzgado");
		$juzgados->execute();	
		$i=0;
	...
	/** Se obtiene un vector de oficios en una fecha **/
	public function obtenerOficiosTelegramas($id) {	
		$fechai   = $_POST['fechai'];
		$fechaf   = $_POST['fechaf'];
		$fechas = $this->db->prepare("select fecha, count(esOficio_Telegrama) as cantidad from(select co.fecha,juz.nombre,co.esOficio_Telegrama,co.idmunicipio,juz.id,co.oficio_telegrama from correspondencia_otros co inner join pa_juzgado juz on (juz.id=co.idjuzgado)where juz.id='$id' and  co.fecha BETWEEN '$fechai' and '$fechaf' union all select act.fecha_envio as fecha,juz.nombre,act.esoficio_telegrama,act.idmunicipio,juz.id,act.oficio_telegrama from correspondencia_tutelas ct inner join pa_juzgado juz on (juz.id=ct.idjuzgado) inner join accionante_accionado_vinculado acc on (acc.idcorrespondencia_tutelas=ct.id) inner join actuacion_tutela act on (act.idaccionado_vinculado_accionante_tut=acc.id) where  act.fecha_envio BETWEEN '$fechai' and '$fechaf' and ct.idjuzgado='$id') x  group by fecha order by fecha;");
		$fechas->execute();
		$i=0;
		$j=0;
		...
		$datos = $this->db->prepare("select co.fecha, juz.nombre,co.esOficio_Telegrama,co.idmunicipio,co.oficio_telegrama from correspondencia_otros co inner join pa_juzgado juz on (juz.id=co.idjuzgado)  where juz.id='$id' and  co.fecha BETWEEN '$fechai' and '$fechaf' union select act.fecha_envio as fecha,juz.nombre,act.esoficio_telegrama,act.idmunicipio,act.oficio_telegrama from correspondencia_tutelas ct inner join pa_juzgado juz on (juz.id=ct.idjuzgado) inner join accionante_accionado_vinculado acc on (acc.idcorrespondencia_tutelas=ct.id) inner join actuacion_tutela act on (act.idaccionado_vinculado_accionante_tut=acc.id) where act.fecha_envio BETWEEN '$fechai' and '$fechaf' and ct.idjuzgado='$id' order by fecha");
		$datos->execute();
